...
#x1r = thr
#x2r = [xr[i]*cos(thr[i]) + yr[i]*sin(thr[i]) for i in range(len(t))]
#x3r = [xr[i]*sin(thr[i]) - yr[i]*cos(thr[i]) for i in range(len(t))]
#plt.plot(t, x1r, 'b', linewidth = 2, label='Ref')
...
for i in range(len(t)): 
    if i == 0:
        ...
        x.append(0); y.append(0); th.append(0)
        x1.append(x[-1]); x2.append(tan(th[-1])); x3.append(y[-1])
        #x1.append(th[-1]); x2.append(x[-1]*cos(th[-1]) + y[-1]*sin(th[-1])); x3.append(x[-1]*sin(th[-1]) - y[-1]*sin(th[-1]))
...
plt.figure()
plt.plot(t, xr, 'b', linewidth = 2, label='Ref')    
plt.plot(t, x, 'r', linewidth = 2, label='Actual')  
plt.legend(loc = 2)
